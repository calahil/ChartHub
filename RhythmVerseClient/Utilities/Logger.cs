...
namespace RhythmVerseClient.Utilities
{
    ...
    public static class Logger
    {
        ...
        private static string _logDirectory = Path.Combine(
            ...
            "RhythmVerseClient",
            "logs");
        private static string _logFilePath = Path.Combine(_logDirectory, "rhythmverseclient.log");
        ...
        public static void Initialize(string? logDirectory = null, IReadOnlyDictionary<string, object?>? context = null)
        {
            lock (Sync)
            {
                if (!string.IsNullOrWhiteSpace(logDirectory))
                {
                    ...
                    _logFilePath = Path.Combine(_logDirectory, "rhythmverseclient.log");
                }
            ...
            IReadOnlyDictionary<string, object?>? context)
        {
            ...
            var redactedMessage = RedactSensitiveText(message);
        ...
        private static string FormatContextValue(string key, object? value)
        {
            ...
            if (LooksLikeUriKey(key) && TryRedactUri(stringValue, out var redactedUri))
        ...
        {
            var redacted = SensitiveAssignmentPattern.Replace(value, "$1$2[REDACTED]");
        ...
        private static void ReportWriteFailure(Exception exception)
        {
            ...
            Trace.WriteLine($"RhythmVerseClient logger write failure: {exception}");
        }
    }
}