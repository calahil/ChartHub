...
public sealed class JsonAppConfigStore : IAppConfigStore
{
    ...
    public JsonAppConfigStore(string configPath)
    {
        ...
        var directory = Path.GetDirectoryName(configPath) ?? throw new ArgumentException("Config path must include a directory.", nameof(configPath));
    ...
    public AppConfigRoot Load()
    {
        ...
        var config = JsonSerializer.Deserialize<AppConfigRoot>(json, _serializerOptions) ?? new AppConfigRoot();
