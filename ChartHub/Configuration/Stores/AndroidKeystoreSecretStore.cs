...
public class AndroidKeystoreSecretStore : ISecretStore
{
    ...
    {
        Android.Content.Context appContext = Application.Context
    ...
    public Task<string?> GetAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        string? payload = _preferences.GetString(key, null);
    ...
    public Task SetAsync(string key, string value, CancellationToken cancellationToken = default)
    {
        ...
        using ISharedPreferencesEditor editor = _preferences.Edit()
    ...
    private string Encrypt(string plaintext)
    {
        ...
        byte[] iv = cipher.GetIV() ?? throw new InvalidOperationException("Android keystore did not provide an IV.");
    ...
    {
        byte[] envelope = Convert.FromBase64String(payload);
    ...
    {
        using KeyStore keyStore = KeyStore.GetInstance("AndroidKeyStore")
            ?? throw new InvalidOperationException("Unable to access Android keystore.");
        keyStore.Load(null);

        if (!keyStore.ContainsAlias(_keyAlias))
        {
            using KeyGenerator keyGenerator = KeyGenerator.GetInstance(KeyProperties.KeyAlgorithmAes, "AndroidKeyStore")
