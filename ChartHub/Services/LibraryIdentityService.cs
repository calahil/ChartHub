...
public static class LibraryIdentityService
{
    ...
    public static string BuildSourceKey(string source, string? sourceId)
    {
        ...
        string normalizedSourceId = string.IsNullOrWhiteSpace(sourceId) ? "unknown" : EscapeComponent(sourceId.Trim());
    ...
    {
        string artist = NormalizeIdentityComponent(metadata.Artist);
        string title = NormalizeIdentityComponent(metadata.Title);
        string charter = NormalizeIdentityComponent(metadata.Charter);
    ...
    public static async Task<string> ComputeInstalledContentIdentityHashAsync(string installDirectory, CancellationToken cancellationToken = default)
    {
        ...
        foreach (string? file in files)
        {
            ...
            string relativePath = Path.GetRelativePath(installDirectory, file).Replace('\\', '/').ToLowerInvariant();
            string fileHash = await ComputeFileSha256HexAsync(file, cancellationToken).ConfigureAwait(false);
