...
public class GoogleDriveWatcher : IResourceWatcher, INotifyPropertyChanged, IAsyncDisposable
{
    ...
    private async Task LoadItemsAsync(CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<WatcherFile> mergedItems = await BuildMergedItemsAsync(Data, files);
        ...
        await Dispatcher.UIThread.InvokeAsync(() =>
        {
            ...
            foreach (WatcherFile? item in toRemove)
            {
                Data.Remove(item);
            }
            ...
            foreach (WatcherFile item in mergedItems)
            {
        ...
        IList<Google.Apis.Drive.v3.Data.File> files)
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(file.Id) && currentById.TryGetValue(file.Id, out WatcherFile? existing))
            {
                merged.Add(existing);
                continue;
            }
